...
export function scrollTop(isCustom = true, speed = 50) {
    let des =
        isCustom && document.querySelector('main-content')
...
export const resizeHandler = function () {
    ...
    bodyClasses = bodyClasses.value
        .split(' ')
        .filter((item) => initClasses.indexOf(item) === -1);
...
export const stickyHeaderHandler = function () {
    ...
    let stickyType =
        window.innerWidth >= 992 ? '.desktop-sticky' : '.mobile-sticky';
    if (
        document.querySelector('header .sticky-header') &&
        !document.querySelector('header .sticky-header.mobile-sticky')
    ) {
        stickyType = '';
    } else {
        if (window.innerWidth >= 992) {
            document.querySelector('.sticky-header.mobile-sticky') &&
                document
                    .querySelector('.sticky-header.mobile-sticky')
                    .classList.remove('fixed');
        } else {
            document.querySelector('.sticky-header.desktop-sticky') &&
                document
                    .querySelector('.sticky-header.desktop-sticky')
                    .classList.remove('fixed');
            document.querySelector('.sticky-wrapper.desktop-sticky') &&
                document
                    .querySelector('.sticky-wrapper.desktop-sticky')
    ...
    if (window.pageYOffset >= top) {
        if (stickyHeader) {
            ...
            if (!document.querySelector('.sticky-wrapper' + stickyType)) {
                ...
                newNode.className =
                    'sticky-wrapper ' +
                    stickyType.substring(1, stickyType.length);
                stickyHeader.parentNode.insertBefore(newNode, stickyHeader);
                document
                    .querySelector('.sticky-wrapper' + stickyType)
